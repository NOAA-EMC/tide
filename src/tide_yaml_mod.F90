...
module tide_yaml_mod
  ...
  !> @brief Fortran representation of the C tide_output_config_t struct.
  type, bind(c) :: tide_output_config_t
    type(c_ptr) :: target_grid_file       !< Path to SCRIP/GRIDSPEC file
    type(c_ptr) :: output_file            !< Path to output NetCDF file
    integer(c_int) :: output_frequency    !< Write interval in seconds
    type(c_ptr) :: regrid_method          !< "bilinear", "neareststod", "nearestdtos", "conserve"
    type(c_ptr) :: output_fields          !< List of field names to output (char**)
    integer(c_int) :: num_output_fields   !< Length of output_fields array
    integer(c_int) :: output_enabled      !< 1 if output block present, 0 otherwise
  ...
  type, bind(c) :: tide_stream_config_t
    ...
    type(tide_output_config_t) :: output !< Output/regrid configuration
  ...
  interface
    ...
    !> @brief Pretty-prints a tide_output_config_t as a YAML string.
    !> @param oc Pointer to the tide_output_config_t struct.
    !> @return Pointer to a newly allocated C string (caller must free), or C_NULL_PTR if disabled.
    function tide_output_config_to_yaml(oc) bind(c, name="tide_output_config_to_yaml")
      use, intrinsic :: iso_c_binding
      type(c_ptr), value :: oc  !< Pointer to tide_output_config_t
      type(c_ptr) :: tide_output_config_to_yaml !< Returned YAML C string
    end function tide_output_config_to_yaml
  end interface
